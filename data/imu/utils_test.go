	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			correctedAcceleration := computeCorrectedGForce(
				NewAcceleration(test.xAcceleration, test.yAcceleration, test.zAcceleration, m, time.Now().UTC()),
				test.xAngle,
				test.yAngle,
				test.zAngle,
			)
...
func Test_ComputeTiltAngles(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			acceleration := NewAcceleration(test.xAxis, test.yAxis, test.zAxis, m, time.Now().UTC())
