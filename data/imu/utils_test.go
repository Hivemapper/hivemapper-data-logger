	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			correctedAcceleration := computeCorrectedGForce(
				NewAcceleration(test.xAcceleration, test.yAcceleration, test.zAcceleration, m, 0.0, time.Now()),
				test.xAngle,
				test.yAngle,
				test.zAngle,
			)
...
func Test_ComputeTiltAngles(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			acceleration := NewAcceleration(test.xAxis, test.yAxis, test.zAxis, m, 0.0, time.Now())
